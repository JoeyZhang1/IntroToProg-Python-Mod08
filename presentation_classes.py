...
class IO:
    ...
    @staticmethod
    def input_employee_data(employee_type: object):
        """ This function gets the first name, last name, and GPA from the user

        ChangeLog: (Who, When, What)
        JZhang,6.8.2025,Created function

        :param employee_type: type of the employee data
        :return: list
        """

        try:
            # Input the data
            employee_type.first_name = input("What is the employee's first name? ")
            employee_type.last_name = input("What is the employee's last name? ")
            employee_type.review_date = input("What is their review date? ")
            employee_type.review_rating = int(input("What is their review rating? "))
